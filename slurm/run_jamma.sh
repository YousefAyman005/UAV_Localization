...
apptainer run --nv \
    ...
    /opt/uav_localization/jamma_pipeline.py \
        --weights /opt/uav_localization/weights/jamma.ckpt \
        --flights all
APPTAINER_EXIT=$?

cd "${LOCAL_JOB_DIR}"
tar -cf "zz_${SLURM_JOB_ID}_jamma.tar" job_results
cp "zz_${SLURM_JOB_ID}_jamma.tar" "${SLURM_SUBMIT_DIR}/"
