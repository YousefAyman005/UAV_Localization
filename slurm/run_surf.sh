...
apptainer run --nv \
    ...
    /opt/uav_localization/pipelines/surf_pipeline.py \
        --workers 10 \
...
cd "${LOCAL_JOB_DIR}"
tar -cf "zz_${SLURM_JOB_ID}_surf.tar" job_results
cp "zz_${SLURM_JOB_ID}_surf.tar" "${SLURM_SUBMIT_DIR}/tar/"
